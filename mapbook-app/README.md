...
When dealing with sensitive data it is advised not to store credentials on a mobile device.  In these cases you authenticate the user, get credentials, and keep it in memory.  When a user closes the app, the memory is cleaned up, and the credentials will be gone forcing users to log into the app every time they open it. This is a good thing, but not necessarily from a usability perspective as there are instances when building an app where the compromise of minimizing the frequency of asking for user credentials is a higher priority.  The MapBook app is demonstrating this type of app and shows the pattern to store an in-memory cache of the users credentials on the device and minimize the frequency of asking for user credentials.  The `DefaultAuthenticationManager` caches credentials in memory by default and can reduce prompts for credentials. It is not recommended to store users password under any circumstance which this app doesn't do, instead it uses an OAuth2 workflow of access and refresh tokens to allow the user to remain authenticated.  Then the app stores the credential cache using a `KeyStore` and encrypts using [Advanced Encryption Standard AES] (https://en.wikipedia.org/wiki/Advanced_Encryption_Standard).  We use this workflow as it is the only app using the credential to allow users to get data updates without explicitly requiring the user to log in.     
